...
"""
1. **입력 처리**:
   - N x N 크기의 격자에서 상어들의 초기 위치와 방향을 입력받고, 상어의 이동 우선순위 정보를 입력받습니다.
   - 격자의 각 칸에는 상어가 있을 경우 해당 상어 번호, 냄새 번호, 냄새 지속 시간을 기록합니다.
   
2. **이동 로직**:
   - 상어는 우선순위에 따라 빈칸이나 자신의 냄새가 있는 칸으로 이동합니다.
   - 먼저 빈 칸을 탐색하고, 빈 칸이 없으면 자신의 냄새가 남아있는 칸을 탐색합니다.
   - 한 칸에 두 마리 이상의 상어가 도착하면, 번호가 작은 상어가 살아남고 나머지는 제거됩니다.
   - 이동한 상어는 자신의 냄새를 새로 남기고, 기존 위치의 냄새는 시간이 줄어들거나 사라집니다.
   
3. **냄새 처리**:
   - 상어가 없는 칸이라도 냄새가 남아 있으면 냄새의 지속 시간이 줄어듭니다. 만약 시간이 0이 되면 냄새가 사라집니다.
   
4. **반복**:
   - 매 라운드마다 모든 상어가 우선순위에 따라 이동합니다.
   - 이동 후, 상어의 마리 수가 1이 되거나 시간이 1000이 넘으면 시뮬레이션을 종료합니다.
   
5. **결과 출력**:
   - 상어가 1마리만 남으면 그때까지 걸린 시간을 출력합니다.
   - 시간이 1000을 넘도록 상어가 2마리 이상 남아있다면 -1을 출력합니다.
"""
